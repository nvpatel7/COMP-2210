...
public class Student implements Comparable<Student> {
   ...
   public int compareTo(Student s) {
      int result = this.lname.compareTo(s.lname); // asc order of last name 
      
      if (result == 0) {
         result = this.fname.compareTo(s.fname); // asc order of first name
      }
      
      if (result == 0) {
         result = this.section - s.section; // asc order of section 
      }
